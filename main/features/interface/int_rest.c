...
static esp_err_t restHandler_schedule(httpd_req_t *req)
{
    char query[128];
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK &&
        ...
        automationTimebased_addRule(hour, minute, id, state);

        httpd_resp_sendstr(req, "OK: time rule added");
        return ESP_OK;
    }
...
static esp_err_t restHandler_scheduleList(httpd_req_t *req)
{
    ...
    for (int i = 0; i < count; i++) {
        time_rule_t *rule = automationTimebased_getRule(i);
...
static esp_err_t restHandler_scheduleRemove(httpd_req_t *req)
{
    ...
    if (httpd_req_get_url_query_str(req, query, sizeof(query)) == ESP_OK &&
        ...
        automationTimebased_removeRule(index);
